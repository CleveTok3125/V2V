...
func Verify(p VerifyParams) (*VerifyResult, error) {
	...
	// encoding so old history and old links keep verifying read-only. New
	// signatures always bind a nonzero TmpID and never match the legacy
	// encoding, so the fallback cannot be abused to strip IDs.
	payload := tripcolor.CanonicalPayload(serverPub, p.Seq, prevBytes, msgHashBytes, pubBytes, p.DisplayName, p.TmpID, p.ReplyTo)
	if p.TmpID == 0 && p.ReplyTo == 0 {
