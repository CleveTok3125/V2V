...
func signFixture(t *testing.T, tmpID uint64, legacy bool) (VerifyParams, []byte) {
	t.Helper()
	seed := make([]byte, 32)
	for i := range seed {
		seed[i] = byte(i + 3)
	...
	if legacy {
		...
		payload = tripcolor.CanonicalPayload(serverPub, 1, prev[:], msgHash[:], pub, "Bob", tmpID)
	}
