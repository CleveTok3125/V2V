...
- Hard policy: user verification required, ES256-only, credential IDs clamped to 1023 bytes, claimed ID must match the parsed credential. Attestation is requested as `direct` but any format (including `none`) is accepted and recorded per credential — synced providers ship no vendor chain, so format is provenance metadata, not a gate.
- Attestation chains are intentionally not verified: synced providers (iCloud Keychain, Bitwarden, 1Password) ship no vendor CA chain, so chain-of-trust would lock out the most common authenticators without adding anything over ceremony binding + UV + ES256 + counter. MDS is out of scope by design, not by omission.
