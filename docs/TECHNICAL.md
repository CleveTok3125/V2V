...
- Secret handling: `-t` takes no value. Resolution order is `V2V_TRIPCODE` env (CI only) > `tripcode.json` in the config dir > hidden interactive prompt. The file holds `{"version":3,"tripcode":"..."}` in the same encrypted v3 envelope (argon2id + XChaCha20Poly1305) as `key.json`, unlocked via `V2V_PASSPHRASE` env or hidden prompt; plaintext files are refused fail-closed and nothing is ever saved unencrypted (empty unlock skips saving). First manual entry offers an encrypted save (`y/N`, default No). Pre-loop prompts share one unbuffered stdin reader so piped answers never starve later readers (prompts, then readline's chat loop).
