...
- `/info <height>[:hash]` prints the full metadata detail of one indexed wire (height/tmp/reply, full hashes, trip fields with live signature verdict, chain verdict, first text line). The trip section shows every signature input (pub, seq, prev, sig, msg_hash with text-match mark, server_pub, payload bytes), so the verdict is checkable by hand with any ed25519 tool. Wires index by height (cap 1000 FIFO); legacy and evicted report as missing.
