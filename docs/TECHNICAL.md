...
- `/reply <height>[:hash] <text>` quotes a buffered message (see above); the height suffix acts as a typo checksum. Quotes resolve per receiver (`wireIdx` first for time/author/verdict, buffer-head fallback) and render `↩ #height | time author ✓/✗: text…` in placeholder and echo alike; the verdict recomputes the target's chain content locally.
